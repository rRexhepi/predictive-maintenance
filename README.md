...
numbers sit near zero. Point it at live telemetry and it earns its
keep. Threshold convention: `PSI < 0.1` = no drift, `0.1–0.25` =
...
Prometheus scrapes the API's `/metrics` directly on a 15s interval, no
Pushgateway needed (the API is long-lived). Grafana auto-loads
